...
/* default timeout in seconds. If it is set to 0, it is turned off */
...
void
test_set_timeout(int sec)
{
	if (!timeout) {
		fprintf(stderr, "Timeout is disabled. "
				"Setting new timeout to %d suppressed\n", sec);
		return;
	}
				
	alarm(sec);
	fprintf(stderr, "Timeout set to %d sec from now.\n", sec);
}
...
int main(int argc, char *argv[])
{
	...
	if ((to_env = getenv("WAYLAND_TEST_TIMEOUT"))) {
		/* XXX use strtol */
		timeout = atoi(to_env);
	}
